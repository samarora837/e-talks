...
//	$("#justtest").val(id);
    ...
    {   
         ...
         if(id == popups[iii])
        {
            ...
            var url='<%=request.getContextPath()%>/parent/parentChatReadStatus';
				$.ajax({
				url:url,
				method:'GET',
				async :false,
				data:{supportId:id,role:role},
...
function endChatSession(chatSessionId,action){
	   var url='<%=request.getContextPath()%>/parent/EndParentSupportChatSession';
		$.ajax({
			url:url,
			method:'GET',
			async :false,
