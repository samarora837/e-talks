            ...
            //	$("#justtest").val(id);
                ...
                {   
                     ...
                     if(id == popups[iii])
                    {
                         ...
                         // ajax to mark message as read in case of popup already opened before in current session
                        var url='<%=request.getContextPath()%>/student/markChatStatusRead';
                 		...
                 		// ajax ENDS to mark message as read in case of popup already opened before in current session 	
               ...
               // 	$("#chatTutor"+id).removeAttr('href');
                ...
                if(tutorProfileId!='NA' && role=='tutor')
                	{
               ...
               // element = element + '<div class="chat-bookSession"><a onclick="openBookingPopUp('+id+');" class="greenBtn-small"> <spring:message code="book.session"/>    </a></div>';
                	 ...
                	 element = element + '<div class="chat-bookSession"><select id="bookingSelect" onchange="openBookPopUp('+id+');" class="chatBookNewSession">';
                	 element = element + '<option value="0" style="background-color: gray;"><spring:message code="student.booknewsession"/></option>';
                	 element = element + '<option value="1" style="background-color: gray;"><spring:message code="book.session"/></option>';
                	 element = element + '<option value="2" style="background-color: gray;"><spring:message code="review.session"/></option>';
...
function openBookPopUp(id){
	...
	var selectBooking=$("#bookingSelect").val();
	if(selectBooking=='1'){
		openBookingPopUp(id);
	}
