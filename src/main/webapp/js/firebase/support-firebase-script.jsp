    ...
    function register_popup(id, name,sessionId)
    {
        ...
        {   
            ...
            if(id == popups[iii])
            {
	           	...
	           	var url='<%=request.getContextPath()%>/cus-care/supportChatStatus';
              	$.ajax({
              		url:url,
              		method:'GET',
              		async :false,
              		data:{supportUserId:userIdSupport,otherUsersId:id},
              		success:function(response){
              		}
              	    });
...
var url='<%=request.getContextPath()%>/cus-care/EndSupportChatSessionAction';
$.ajax({
	url:url,
	method:'GET',
	async :false,
  ...
  $('.trying').keypress(function (e) {
	  ...
	  var msgVal = $("#"+textBoxId).val();
        if ((e.keyCode == 13 || typeof e.which === "undefined") && msgVal.trim()!="") {
          ...
          var url='<%=request.getContextPath()%>/cus-care/replyToChatStatus';
         	$.ajax({
         		url:url,
         		method:'GET',
         		async :false,
         		data:{supportUserId:supportUserId,otherUsersId:otherUsersId},
         		success:function(response){
         		}
         	    });
