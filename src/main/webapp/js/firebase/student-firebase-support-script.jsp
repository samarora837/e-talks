...
function register_popup(id, name,role,supportProfileId,chatsessionId)
{
	$("#userRole").val(role);
	var chatSatus="N";
	if($("#"+id).length!=0){
...
//	$("#justtest").val(id);
    ...
    {   
         ...
         if(id == popups[iii])
        {
        		...
        		var url='<%=request.getContextPath()%>/student/markChatStatusRead';
        	  	$.ajax({
        	  		url:url,
        	  		method:'GET',
        	  		async :false,
        	  		data:{tutorId:id,role:role},
   ...
   // 	$("#chatTutor"+id).removeAttr('href');
    ...
    element = element + '<div class="popup-head-right"> <a class="minimize_chat" href="javascript:close_popup(\''+ id +'\','+chatsessionId+',\'minimize\');">_</a> <a href="javascript:close_popup(\''+ id +'\','+chatsessionId+',\'close\');">&#10005;</a></div>';
