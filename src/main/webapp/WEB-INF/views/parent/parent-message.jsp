 ...
 <c:if test="${showPopup eq 'true' }">
   ...
   <div class="form-tutor-popup my-info-popup-outr">
   		<div class="fTutor-popup-txt">
        	<div class="close-icon"><a href="<%=request.getContextPath()%>/parent/messages"><img src="<%=request.getContextPath()%>/images/close-icon.png" alt=""></a></div>
...
function openSendMessagePopUp()
{
	...
	var url='<%=request.getContextPath()%>/parent/getAllEmail';
	$.ajax({
		url:url,
		method:'POST',
		async :false,
		success:function(response){
			if(response!=null){
				...
				/*  $('#toId').append('<optgroup label="Parents">');
...
function submitSendMessageForm(){
	...
	$("#sendMessageForm").attr("action", "<%=request.getContextPath()%>/parent/sendMessage");
...
function submitReplyMessageForm(){
	...
	$("#replyMessageForm").attr("action", "<%=request.getContextPath()%>/parent/sendReply");
