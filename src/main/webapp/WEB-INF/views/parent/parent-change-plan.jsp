...
function subscribePlan(){
	...
	var url='<%=request.getContextPath()%>/parent/subscription';
	$.ajax({
		url:url,
		method:'GET',
		async :false,
		data:{planId:planId,selectDuration:selectDuration,studentId:studentId},
...
<%@ include file="/WEB-INF/views/parent/parent-inner-header.jsp" %>
  <!--Mid Section-->
    <section class="container">
 			...
 			<div class="std-detail_outr">
 			<h1><spring:message code="student.details"/></h1>
 			<div class="std-detail_txt">
 				<h3>${studentName}</h3>
            	<div><spring:message code="student.plan"/> : ${planName}</div>
            	<div><spring:message code="student.mimbalance"/> : ${minBalance} <spring:message code="mins"/></div>
...
function getBuyMinute(){
	...
	$.ajax({
        url: "<%=request.getContextPath()%>/parent/pay",
        method:'GET',
		async :false,
		data:{buyMinute:buyMinute,studentId:studentId},
...
function cancelSubscription(){
	var studentId=$("#studentId").val();
	var url='<%=request.getContextPath()%>/parent/cancelSubscription';
	$.ajax({
		url:url,
		method:'GET',
		async :false,
