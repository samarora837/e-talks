...
.you::before {
  ...
  <%@ include file="/js/firebase/student-firebase-support-script.jsp" %>
...
<script type="text/javascript">
 ...
 function sendMsg(fixit){ 
  ...
  // // console.log("Text:", text);
    if(text!=null){
  	  window['newUserRef' + supportUserId ].push({name: "tutor", text: text});
    $('#chatMsg'+fixit).val('');
    ...
    var studentUserId = elementIdVal.substring(0, elementIdVal.indexOf('-'));
    var cusSupporrtUserId = elementIdVal.substring(elementIdVal.indexOf('-')+1,elementIdVal.length);
	  window['newUserRef' + supportUserId ]=new Firebase('https://aloprofe.firebaseio.com/users/'+studentUserId +':'+cusSupporrtUserId);
   ...
   window['newUserRef' + supportUserId ].push({name: name, text: text});
	...
	$.ajax({
		...
		method:'GET',
		async :false,
		data:{tutorId:supportUserId,role:role},
...
//var posfix = stId+"-";
var newUserRef ="";
function openchatwind(supportUserId,chatSatus,role){
	
	studentId1='${studentId}';
	$("#justtest").val(supportUserId);
	var url='<%=request.getContextPath()%>/student/requestTutorChat';
	$.ajax({
		url:url,
		method:'GET',
		...
		var posfix =studentId1+"-"+supportUserId;
		
		
		
		if(chatSatus=='Y'){
			window['newUserRef' + supportUserId ]=new Firebase('https://aloprofe.firebaseio.com/users/'+ studentId1 +':'+ supportUserId);
		 
		 window['newUserRef' + supportUserId ].on('child_added', function(DataSnapshot) {
      ...
      <div class="form-tutor-con">
        <div class="stu-ac-outr"> 
            ...
            <div style="width: 100%; height: 400px; overflow: auto;">
           	...
           	<a href="javascript:register_popup('${dtoCustomerSupportDetails.supportUserId}', '${dtoCustomerSupportDetails.supportName}', 'support', '${dtoCustomerSupportDetails.supportProfileId}', '${dtoCustomerSupportDetails.chatSessionId}');" name="chatTutor${dtoCustomerSupportDetails.supportUserId}" id="chatTutor${dtoCustomerSupportDetails.supportUserId}"  class="greenBtn-normal"><spring:message code="chatWithTutor"></spring:message> </a>
...
<%@ include file="/WEB-INF/views/footerInner.jsp" %>
       <input type="hidden" id="userRole" readonly="readonly"/> 
       <input type="hidden" id="receipentRoleName" nmae="receipentRoleName" readonly="readonly"/> 
</body>
<script type="text/javascript">

<%-- setInterval(function(){
	var url='<%=request.getContextPath()%>/student/customerStudentSupportLoginStatus';
	$.ajax({
		url:url,
		method:'GET',
		async :false,
...
setInterval(function(){
	var url='<%=request.getContextPath()%>/student/getStudentSupportChatMessageStatus';
	$.ajax({
		url:url,
		method:'GET',
		async :false,
		success:function(response){
			if(response != null && response.modelMap.dtoStudentDetails != null){
				var recordCounter=0;
				$.each( response.modelMap.dtoStudentDetails, function( key , value ) {
					 
					 if(value.readByUSer=='N'){
