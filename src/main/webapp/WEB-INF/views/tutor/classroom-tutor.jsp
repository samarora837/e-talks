...
<section class="container">
  <div class="inner-row">
    <div class="classroom-stu-top">
      <ul>
        <li style="width:30%; text-align: left;"><span><spring:message code="student" /> :</span> ${dtoScribblarMeeting.studentFormattedName }</li>
        ...
        <li style="width:24%; text-align: left;"><span><spring:message code="session.time" /> :</span> ${dtoScribblarMeeting.bookingDuration} <spring:message code="mins"/></li>
...
$(document).ready(function() {
	...
	var tutorJoined = '${dtoScribblarMeeting.isTutorJoined}';
	if(tutorJoined=='Y'){
...
function startSession(bookingId){
	 ...
	 var url='<%=request.getContextPath()%>/tutor/startTutorSessionTime';
	$.ajax({
		url:url,
		method:'GET',
		async :false,
	 ...
	 var url='<%=request.getContextPath()%>/checkStudentTutorAvailability';
		$.ajax({
			...
			success:function(response){
				...
				/* else{
					$('#meetingTimer').html('<spring:message code="waitingforstudent"/>');
				} */
				
				else{
					if(response.modelMap.studentJoined=='Y'){
						$('#meetingTimer').html('<spring:message code="studentConnected"/>');
					}else{
						$('#meetingTimer').html('<spring:message code="waitingforstudent"/>');
					}
				}
			...
			success:function(response){
				...
				if(response.modelMap.leaveflag==true){
				  ...
				  //  $('#meetingTimer').html('Tutor is not longer connected');
				    $('#remainTime').html('<span><spring:message code="student.notconnected" /> </span> ');
				    refreshStatusFlag="0";
				}
				}
			}
...
flashvars.username = '${dtoScribblarMeeting.tutorFormattedName}';
...
swfobject.embedSWF("http://media.muchosmedia.com/scribblar/v4/main.swf", "alternate", "100%", "100%", "11.1.0", "http://media.muchosmedia.com/swfobject/expressInstall.swf", flashvars, params, attributes);
  ...
  setInterval(function(){
		...
		var url='<%=request.getContextPath()%>/tutor/setMeActive';
		$.ajax({
			url:url,
			method:'GET',
			async :false,
